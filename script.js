...
function nextQuestion(){
    ...
    if(currentQuestion == questionsNum){
        ...
        calculateResults();
        setTimeout(() => {
            changeWrapper(resultWrapper, quizWrapper);
...
function enterDatabase(){
    ...
    if(localStorage.getItem("username")){
        ...
        if(logsInputs[2].value != customPin){
            flashInput(inpWrappers[2], 1);
        }
        if(logsInputs[0].value.length > 0 && logsInputs[1].value.length > 0 && logsInputs[2].value == customPin){
